...
impl ListHead {
    ...
    /// 在当前节点之后插入一个新节点
    pub unsafe fn add(&mut self, new: *mut ListHead) { unsafe {
        let next = self.next;
        (*new).next = next;
    ...
    pub unsafe fn add_rcu(&mut self, new: *mut ListHead) { unsafe {
        let next = self.next;
        (*new).next = next;
        (*new).prev = self as *mut _;
    ...
    /// 从双向链表中删除当前节点
    pub unsafe fn del(&mut self) { unsafe {
        let next = self.next;
        let prev = self.prev;
        (*next).prev = prev;
        (*prev).next = next;
    ...
    pub unsafe fn del_rcu(&mut self) { unsafe {
        let next = self.next;
        let prev = self.prev;
    ...
    pub unsafe fn next_rcu(&self) -> *mut ListHead { unsafe {
